...
*/
/*
This script is called from the CGI control, after loading import_api.php.
...
if (! $err) // pull in other configuration and check for required input
{
	...
	if (! ($job && $id && $type)) $err = 'Parameters job, id, type required';
}
...
if (! $err)
{
	...
	if ($progress['percent'] < 100) $attrs = ' delay="10" url="media/php/import_monitor.php?type=' . $type . '&amp;job=' . $job . '&amp;id=' . $id . '"';
	else $attrs = ' trigger="browser.parameters.group=' . $type . '"';
}
